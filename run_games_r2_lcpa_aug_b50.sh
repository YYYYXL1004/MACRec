#!/bin/bash
# Games R2 RQVAE + 轻量CPA + 序列增强 + beam-50
# 参照 Instruments 最佳配置: LCPA w=0.005 cosine + aug dropout=0.1 crop=0.3 + beam-50
# RQVAE索引: R2 (LLaMA+concat+CAQ, 碰撞率 0.47%/0.40%)
# GPU: 0,3 双卡（1,2 被 Arts 占用）
...
OUTPUT_DIR="/sda/data/yaoxianglin/MACRec/log/${DATASET}-${SAVE_NAME}-lcpa${CPA_W}-aug-b${NUM_BEAMS}"

cd /sda/data/yaoxianglin/MACRec

echo "============================================"
echo " Games R2 + LCPA w=${CPA_W} + Aug (dp=${AUG_DROPOUT},crop=${AUG_CROP}) + beam-${NUM_BEAMS}"
...
echo "============================================"
echo " 全流程完成！$(date)"
echo " 配置: R2 + LCPA w=${CPA_W} ${CPA_LOSS} + Aug dp=${AUG_DROPOUT} crop=${AUG_CROP} + beam-${NUM_BEAMS}"
echo "============================================"
