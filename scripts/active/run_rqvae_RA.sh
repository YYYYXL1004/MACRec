#!/bin/bash
# RQVAE R-A: 3层×256 + e_dim=64（减层+增宽）
# RQVAE 单卡 GPU 2, T5 双卡 GPU 2,3 (与 R-B 共享)
# 用法: screen -dmS rq_ra bash scripts/active/run_rqvae_RA.sh
...
echo "============================================"
echo " RQVAE R-A: 3层×256 + e_dim=64"
...
echo "[R-A][Step 1] RQVAE 完成 $(date)"

# Step 2-3: 生成 code
echo "[R-A][Step 2] 生成 text code... $(date)"
...
echo "[R-A][Step 3] 生成 image code... $(date)"
CUDA_VISIBLE_DEVICES=${RQVAE_GPU} python -u generate_indices_distance.py \
    --dataset $DATASET \
    --text_data_path ${TEXT_ST} \
    --image_data_path ${IMAGE} \
...
# Step 5-6: 推理
echo "[R-A][Step 5] seqrec 推理... $(date)"
...
echo "[R-A][Step 6] seqimage 推理... $(date)"
CUDA_VISIBLE_DEVICES=$TRAIN_GPUS torchrun --nproc_per_node=2 --master_port=$PORT test_ddp_save.py \
    --ckpt_path $OUTPUT_DIR \
    --data_path ./data/ \
    --dataset $DATASET \
