#!/bin/bash
# RQVAE R-C: 256×4 + Sinkhorn均衡
# RQVAE 单卡 GPU 0, T5 双卡 GPU 0,1 (与 R-D 共享)
# 用法: screen -dmS rq_rc bash scripts/active/run_rqvae_RC.sh
...
echo "============================================"
echo " RQVAE R-C: 256×4 + Sinkhorn(ε=0.003)"
...
CUDA_VISIBLE_DEVICES=${RQVAE_GPU} python -u main.py \
    ...
    --e_dim 32 \
    --sk_epsilons 0.0 0.0 0.003 0.003 \
    --eval_step 2 \
    --batch_size 2048 \
    --begin_cross_layer 2 \
    --use_cross_rq True \
...
echo "[R-C][Step 1] RQVAE 完成 $(date)"

# Step 2-3: 生成 code
echo "[R-C][Step 2] 生成 text code... $(date)"
...
echo "[R-C][Step 3] 生成 image code... $(date)"
CUDA_VISIBLE_DEVICES=${RQVAE_GPU} python -u generate_indices_distance.py \
    --dataset $DATASET --text_data_path ${TEXT_ST} --image_data_path ${IMAGE} \
...
echo "[R-C][Step 5] seqrec 推理... $(date)"
...
echo "[R-C][Step 6] seqimage 推理... $(date)"
CUDA_VISIBLE_DEVICES=$TRAIN_GPUS torchrun --nproc_per_node=2 --master_port=$PORT test_ddp_save.py \
    --ckpt_path $OUTPUT_DIR --data_path ./data/ --dataset $DATASET --test_batch_size 64 --num_beams 20 \
    --index_file $INDEX_FILE --image_index_file $IMAGE_INDEX_FILE \
