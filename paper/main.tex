...
% 
Our codes are available at \url{XXXX}.
\end{abstract}
...
\begin{figure}[t]
    ...
    \caption{Illustrative comparison on the Instruments dataset. (a)~User interaction history. (b)~Content-only methods rank candidates by text and image similarity: a guitar cable scores high on text similarity and a guitar case scores high on image similarity, yet neither modality surfaces the ground truth (amplifier). (c)~With collaborative anchoring, the collaborative-semantic similarity correctly ranks the amplifier first and retrieves collaboratively related items (e.g., effects pedal) that content-only methods overlook.}
    \Description{Illustrative comparison showing content-only methods fail to rank the ground truth item while collaborative anchoring succeeds.}
    \label{fig:intro}
\end{figure}
\section{Introduction}
Recommendation systems help users navigate information overload across e-commerce~\cite{amazonrec1,taobaorec2,liu2025multi,zhang2022mind}, social media~\cite{davidson2010youtube,dnn}, online advertising~\cite{xi2019modelling,xi2021modeling}, and short video platforms~\cite{zhu2024interest,zhu2025long,bin2025real}.
%
...
%
Each item is encoded as a discrete semantic identifier (ID) through vector quantization, and an autoregressive model generates the next item's semantic ID given the user's interaction history~\cite{tiger}.
%
This formulation leverages the strong sequence modeling capabilities of LLMs~\cite{zhang2025hiergr,zhang2025multi,liu2025cat}.

A central question in GR is how to construct high-quality semantic IDs.
%
Early methods quantize single-modality (text) embeddings through RQ-VAE~\cite{tiger,image_rqvae}.
%
...
Despite this progress, existing multi-modal GR methods build code spaces primarily from content features without explicitly aligning the code structure with collaborative structure.
%
Content similarity, however, does not equal collaborative similarity.
%
As shown in Figure~\ref{fig:intro}, given a user who purchased an electric guitar, a tuner, and a guitar pick, content-only methods rank a guitar cable first (high text similarity due to shared ``guitar'' terminology) and a guitar case second (high image similarity due to guitar-shaped outline), while the ground truth---a guitar amplifier---ranks last because it differs in both text description and visual appearance.
%
With collaborative anchoring, the amplifier rises to the top because users who buy guitars frequently purchase amplifiers next, and the model also surfaces an effects pedal that content-only methods entirely miss.
%
When the code space primarily reflects content proximity, collaboratively related items receive distant codes, and the autoregressive decoder must predict across a topology that conflicts with the recommendation objective.
%
Furthermore, at inference, autoregressive decoding ranks candidates solely by token-level log-probabilities in the discrete code space; two items with nearly identical behavioral patterns can receive very different beam scores if their codes diverge at early positions.
...
\begin{itemize}
    \item We propose collaborative-anchored cross-modal quantization, which concatenates collaborative embeddings from pretrained sequential models with text and image features as the RQ-VAE input, restructuring both code spaces to respect behavioral proximity while preserving per-modality discriminability.
...
Classical sequential recommenders---recurrent models~\cite{gru4rec}, self-attentive Transformers~\cite{sasrec}, masked Transformers~\cite{sun2019bert4rec}, and multi-modal variants~\cite{wei2019mmgcn,wang2023missrec}---rank items by representation similarity in an embed-and-retrieve paradigm.
%
Generative recommendation (GR) departs from this paradigm: each item is encoded as a discrete semantic ID via RQ-VAE~\cite{image_rqvae}, and a sequence-to-sequence model autoregressively generates the target ID from the user's interaction history.
%
...
Items with distinct descriptions and appearances can share similar interaction patterns; content similarity does not equal collaborative similarity.
%
Bridging this gap is critical for GR, because the code topology directly constrains autoregressive prediction---collaboratively related items that receive distant codes force the decoder to predict across a conflicting structure.
...
%
These methods each inject collaborative signals into the code learning process; however, they either operate on a single content modality~\cite{letter,colarec}, compress all signals into one code at the cost of per-modality discriminability~\cite{unger}, or route collaborative signals through a separate quantization path that does not reshape the text or image code topology~\cite{mscgrec}.

Beyond code-space alignment, ETEGRec~\cite{etegrec} addresses the gap between the item tokenizer and the generative recommender through alternating optimization, proposing Sequence-Item Alignment~(SIA)---which minimizes the KL divergence between the recommender encoder's quantized distribution and the target item's token distribution---and Preference-Semantic Alignment~(PSA)---which uses contrastive learning to align the decoder's user-preference representation with the target item's reconstructed semantic embedding.
%
This end-to-end alignment enables tokenizer-recommender co-adaptation, but the code space is still built from single-modality content features and inference remains purely in discrete code space.
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\linewidth]{CAGRec.pdf}
    \caption{Overall architecture of CAGRec. \textbf{Left}: Collaborative-Anchored Cross-Modal Quantization. Collaborative embeddings from a pretrained SASRec are concatenated with text and image features to form anchored inputs for dual-path RQ-VAE. Cross-modal contrastive learning ($\mathcal{L}_{\mathrm{con}}$) aligns residuals across modalities, reconstruction alignment ($\mathcal{L}_{\mathrm{align}}$) keeps the two quantized latents close, and Collaborative-Aware Regularization ($\mathcal{L}_{\mathrm{CAR}}$) reinforces collaborative proximity in the pre-quantization latent space. The decoders output into the collaborative-semantic space. \textbf{Upper right}: Recommendation Training. A T5-based model predicts target item codes ($\mathcal{L}_{\mathrm{code}}$) and is trained with Collaborative Preference Anchoring ($\mathcal{L}_{\mathrm{CPA}}$) that minimizes cosine distance between the decoder's user representation and the target item's collaborative-semantic embedding. \textbf{Lower right}: Semantic-aware Reranking. At inference, the T5 encoder output is projected into the collaborative-semantic space via $f_{\mathrm{dec}}$, and beam search candidates are reranked by combining generation likelihood with cosine similarity.}
    \Description{Architecture diagram of CAGRec with three parts: (left) collaborative-anchored cross-modal RQ-VAE that concatenates collaborative embeddings with text and image features for dual-path quantization with collaborative-aware regularization; (upper right) recommendation training with code prediction and cosine-based collaborative preference anchoring; (lower right) semantic-aware reranking that combines beam search scores with collaborative-semantic cosine similarity.}
    \label{fig:model}
\end{figure*}
...
%
However, its code spaces are built purely from content features: items with similar interaction patterns but distinct text or images end up with distant codes, forcing the downstream generative model to predict across a misaligned code topology.
%
...
% 
We also encode the text description and image of item~$i$ into $\mathbf{t}_i \in \mathbb{R}^{d_t}$ and $\mathbf{v}_i \in \mathbb{R}^{d_v}$ using a pretrained sentence encoder and a vision encoder, respectively.
...
%
Concatenation preserves collaborative and content signals in separate subspaces, letting the MLP encoder learn a flexible combination without collapsing the two.

% ---- 双路 RQ-VAE ----
\subsubsection{Dual-Path Cross-Modal RQ-VAE}

We adopt a dual-path Residual-Quantized Variational AutoEncoder (RQ-VAE) architecture.
% 
...
%
Starting from layer~$L_c$, a cross-modal contrastive loss applies InfoNCE on the RQ residuals using the other modality's K-means pseudo-labels as positive-pair indicators:
\begin{equation}
...
\begin{equation}
    \mathcal{L}_{\text{code}}^m = -\sum_{l=1}^{L} w_l\,\log P_\theta(c_l^m \mid c_{<l}^m,\, \mathbf{x}^m),
\end{equation}
where $w_l$ is the position-specific weight that allows up-weighting early code positions for coarse-level accuracy.
%
...
%
We apply a warmup of $W$ epochs that runs only $\mathcal{L}_{\text{code}}$; $\mathcal{L}_{\text{CPA}}$ is activated afterward to let basic code prediction stabilize first.
...
\end{equation}
Although $f_{\text{dec}}^m$ is trained on decoder hidden states, the T5 decoder's initial state is derived from the encoder output via cross-attention, so the two representations lie in a closely related subspace; we verify the effectiveness of this approximation experimentally in Section~4.
...
\begin{table*}[t!]
\caption{Performance comparison on three datasets. Bold and underline indicate the best and second-best results, respectively. $\dagger$: reproduced by us under identical settings; all other baseline results are cited from~\cite{macrec}. ``--'': experiments not yet conducted.}
\label{tab:main}
\centering
\resizebox{\textwidth}{!}{%
...
& HR@1      & 0.0450 & 0.0318 & 0.0339 & 0.0723 & 0.0512 & 0.0754 & 0.0737 & 0.0763 & \underline{0.0804} & \textbf{0.0826} \\
& HR@5      & 0.0856 & 0.0946 & 0.0937 & 0.1089 & 0.0839 & 0.1007 & 0.0892 & 0.1058 & \underline{0.1112} & \textbf{0.1156} \\
& HR@10     & 0.1081 & 0.1233 & 0.1123 & \underline{0.1361} & 0.1119 & 0.1221 & 0.1071 & 0.1291 & 0.1349 & \textbf{0.1400} \\
& NDCG@5    & 0.0667 & 0.0654 & 0.0693 & 0.0797 & 0.0678 & 0.0882 & 0.0815 & 0.0902 & \underline{0.0962} & \textbf{0.0992} \\
& NDCG@10   & 0.0739 & 0.0746 & 0.0743 & 0.0880 & 0.0704 & 0.0950 & 0.0872 & 0.0997 & \underline{0.1038} & \textbf{0.1070} \\
\midrule
\multirow{5}{*}{Arts}
& HR@1      & 0.0289 & 0.0212 & 0.0172 & 0.0479 & 0.0421 & 0.0532 & 0.0474 & \underline{0.0626} & 0.0611 & \textbf{0.0626} \\
...
\midrule
\multirow{5}{*}{Games}
& HR@1      & 0.0115 & 0.0069 & 0.0136 & \textbf{0.0201} & 0.0169 & 0.0166 & 0.0173 & \underline{0.0200} & 0.0192 & \textbf{0.0201} \\
...
We evaluate CAGRec along five research questions:
...
\textbf{RQ5}:~What does the content-collaborative gap look like empirically?
...
We obtain collaborative embeddings ($d_c\!=\!256$) from a pretrained SASRec~\cite{sasrec} model, text features ($d_t\!=\!4096$) from LLaMA-7B~\cite{touvron2023llama} mean pooling, and image features from ViT-L/14 (CLIP).
% 
...
% 
CPA uses cosine distance with weight $\lambda_p\!=\!0.005$ and a linear adapter projecting from T5 hidden dimension to the 256d SASRec space; CPA activates after $W$ warmup epochs.
% 
We use AdamW (batch size 1024, learning rate $10^{-3}$), beam search with $B\!=\!50$, dual-path score ensemble, and semantic-aware reranking strength $\beta\!=\!0.1$.
% 
...
% 
We reproduce MACRec (marked with~$\dagger$) under its published settings with five random seeds; all other baseline results are cited from the original paper~\cite{macrec}.
%
From the results, we have the following observations.

\noindent$\bullet$~\textbf{CAGRec consistently achieves the best performance across every metric and dataset.}
%
On the primary ranking metric NDCG@10, CAGRec surpasses the strongest baseline by $+2.8\%$--$3.7\%$ across three datasets with varying sparsity levels (99.87\%--99.94\%), demonstrating the robustness and generalizability of collaborative anchoring.

\noindent$\bullet$~\textbf{Collaborative anchoring particularly benefits recall-oriented metrics.}
%
The relative improvements on HR@10 ($+3.6\%$ to $+4.1\%$, averaging $+3.9\%$) consistently exceed those on NDCG@10 ($+2.8\%$ to $+3.7\%$).
%
We attribute this to the restructured code space: by steering collaboratively related items toward proximate codes, the autoregressive model can surface a broader set of relevant candidates, expanding recall without sacrificing ranking precision.

\noindent$\bullet$~\textbf{Multi-modal generative methods significantly outperform traditional sequential models.}
%
MACRec, MQL4GRec, and CAGRec consistently surpass BERT4Rec and SASRec by large margins, confirming the advantage of semantic-ID-based generation with multi-modal features.
%
Among generative baselines, MQL4GRec and MISSRec are the strongest competitors: MQL4GRec surpasses MACRec$^\dagger$ on Arts, and MISSRec leads on Games HR@1.
%
CAGRec outperforms both across all datasets on NDCG, indicating that collaborative anchoring provides complementary benefits beyond what richer content encodings alone can achieve.
...
\midrule
\textbf{CAGRec} (full)  & \textbf{0.1400} & \textbf{0.1070} & --              & --              \\
\;\;w/o Input Anchoring  & 0.1334          & 0.1035          & --              & --              \\
\;\;w/o CAR              & 0.1308          & 0.1012          & --              & --              \\
\;\;w/o CPA              & 0.1369          & 0.1057          & --              & --              \\
\;\;w/o Reranking        & --              & --              & --              & --              \\
\bottomrule
\end{tabular}
\end{table}

We ablate each component from the full CAGRec (Table~\ref{tab:ablation}) to quantify its individual contribution.
To examine whether the conclusions hold across different data scales, we conduct ablations on both Instruments (6{,}250 items, the smallest) and Games (13{,}839 items, the largest).
We select these two datasets because they represent the two extremes of our benchmark in catalog size; Arts (7{,}360 items) falls in between and would add limited additional insight, so we omit it to conserve computational budget.

\noindent$\bullet$~\textbf{w/o Input Anchoring} removes the concatenation of collaborative embeddings at the RQ-VAE input, relying solely on content features for code assignment.
On Instruments, this causes the RQ-VAE collision rate to surge from 5.0\% to 37.6\%, as the quantizer loses the collaborative signal that helps distinguish similar items.
NDCG@10 drops by $-3.3\%$, confirming that direct feature-level anchoring is the primary channel for injecting collaborative knowledge.

\noindent$\bullet$~\textbf{w/o CAR} removes Collaborative-Aware Regularization from RQ-VAE training, eliminating the contrastive constraint that clusters collaboratively related items in the latent space.
This produces the largest degradation: NDCG@10 falls by $-5.4\%$ on Instruments.
Interestingly, without the contrastive constraint, the RQ-VAE collision rate drops to 0.3\% (lower than the full model's 5.0\%), since the quantizer freely minimizes reconstruction loss without the competing collaborative objective.
However, the resulting codes lose collaborative-semantic structure, leading to the worst downstream performance.

\noindent$\bullet$~\textbf{w/o CPA} removes Collaborative Preference Anchoring from downstream T5 training, disabling the auxiliary loss that aligns user representations with collaborative-semantic targets.
NDCG@10 decreases by $-1.2\%$, indicating that CPA provides a complementary refinement at the decoder stage.

\noindent$\bullet$~\textbf{w/o Reranking} disables semantic-aware reranking by setting $\beta\!=\!0$, ranking candidates purely by beam search log-probability without collaborative-semantic similarity.

\noindent Overall, the contribution ranking is CAR $>$ Input Anchoring $>$ CPA $>$ Reranking, showing that the RQ-VAE stage components (CAR and Input Anchoring) are the most critical, while the downstream CPA and inference-time reranking provide incremental but consistent gains.

% ====================================================================
\subsection{Collaborative Embedding Fusion Analysis (RQ3)}
\label{sec:rq3}

We investigate the design choice of how to fuse collaborative embeddings with content features in the RQ-VAE.

\begin{table}[t]
\caption{Comparison of collaborative embedding fusion strategies on Instruments. All variants use the same SASRec collaborative embedding and identical downstream T5 configuration; only the RQ-VAE fusion method differs. \textbf{Coll.} reports the best text / image codebook collision rate during RQ-VAE training (lower is better).}
\label{tab:fusion}
\centering
...
%
Our approach outperforms all parameterized fusion strategies across every metric (Table~\ref{tab:fusion}).
%
On NDCG@10, concatenation surpasses Proj.\ Addition by $+2.9\%$, Gating by $+1.3\%$, and Cross-Attention by $+3.1\%$.
%
More strikingly, the RQ-VAE codebook collision rate is dramatically lower: 5.0\%/2.0\% (text/image) versus 23.9\%--73.3\% / 38.2\%--90.1\% for the alternatives.
...
%
We attribute this to a shared failure mode: when collaborative and content signals interact through learnable parameters \emph{before} quantization, the additional parameters create unstable gradients during codebook updates, causing many items to be mapped to identical codes.

\noindent$\bullet$~\textbf{Collision rate and downstream performance are not perfectly correlated.}
%
Despite having the worst collision rate, Gating outperforms both Proj.\ Addition and Cross-Attention on NDCG@10 (0.1056 vs.\ 0.1040 and 0.1038).
%
This is because the distance-based index reassignment (\S\ref{sec:quant}) resolves collisions post-training by mapping conflicting items to their next-nearest codes, partially compensating for poor codebook quality.
%
Nevertheless, concatenation's inherently low collision rate (5.0\%/2.0\%) avoids the need for such correction, yielding codes that directly reflect the intended collaborative-content structure.

\noindent These results validate our design choice: simple concatenation is not only the most effective fusion strategy but also the most stable, because it preserves the full dimensionality of both signals without introducing learnable fusion parameters that can destabilize RQ-VAE training.

% ====================================================================
\subsection{Hyperparameter Sensitivity (RQ4)}
\label{sec:rq4}

% TODO: 生成超参数分析 Figure，包含4个子图，形式参考 MACRec Figure 3
% 每个子图为柱状图，包含 HR@10（蓝色）和 NDCG@10（橙色）两组柱
% (a) CAR weight λ_r: {0.5, 1.0, 2.0, 5.0}
% (b) CPA weight λ_p: {0, 0.002, 0.005, 0.01}
% (c) alignment weight λ_a: {0.0001, 0.001, 0.01}
% (d) reranking strength β: {0, 0.05, 0.1, 0.2}
\begin{figure*}[t]
    \centering
    % TODO: 替换为实际生成的图片
    % \includegraphics[width=\linewidth]{hyperparam_analysis.pdf}
    \fbox{\parbox{0.95\linewidth}{\centering\vspace{3em}\textbf{TODO: Hyperparameter sensitivity figure}\\\small (a) $\lambda_r$ (CAR) \quad (b) $\lambda_p$ (CPA) \quad (c) $\lambda_a$ (align) \quad (d) $\beta$ (rerank)\vspace{3em}}}
    \caption{Performance of CAGRec over different hyperparameters on Instruments. Each subplot reports HR@10 and NDCG@10.}
    \Description{Four-panel bar chart showing hyperparameter sensitivity of CAGRec on Instruments.}
    \label{fig:hyperparam}
\end{figure*}

Figure~\ref{fig:hyperparam} reports HR@10 and NDCG@10 on Instruments as we vary four key hyperparameters.

\noindent\textbf{CAR weight $\lambda_r$.}
% TODO: 补充趋势分析（预期 λ_r=2.0 最优）

\noindent\textbf{CPA weight $\lambda_p$.}
We test $\lambda_p \in \{0, 0.002, 0.005, 0.01\}$.
% 
$\lambda_p\!=\!0.005$ yields the best NDCG@5 and NDCG@10; at $\lambda_p\!=\!0.002$ the alignment signal is too weak, while at $\lambda_p\!=\!0.01$ the decoder over-optimizes cosine alignment at the cost of code prediction accuracy.
% 
At the optimum, CPA lifts HR@1 by $+1.1\%$ and NDCG@5 by $+1.3\%$ while trading only $-1.1\%$ on HR@10, consistent with its role as a precision-refinement loss.

\noindent\textbf{Alignment weight $\lambda_a$.}
% TODO: 补充趋势分析（预期 λ_a=0.001 最优）

\noindent\textbf{Reranking strength $\beta$.}
We vary $\beta$ in $\{0, 0.05, 0.1, 0.2\}$.
%
At $\beta\!=\!0.05$--$0.1$, NDCG@10 stays at 0.1061; at $\beta\!=\!0.2$ it drops to 0.1055.
% 
Collaborative anchoring already aligns the code space with behavioral structure, so the autoregressive model produces well-calibrated rankings; the continuous similarity signal serves as a tie-breaker rather than a correction.
% 
This confirms that CAGRec's primary gains originate from the improved code space and training, not from post-hoc inference.
...
Existing multimodal generative recommenders build code spaces from content features alone, leaving a gap between code topology and collaborative structure.
% 
We propose CAGRec, which injects collaborative signals at multiple stages: input-level concatenation reshapes the RQ-VAE code space, latent-level regularization (CAR) reinforces behavioral clustering before quantization, and training-level preference anchoring (CPA) grounds user representations in the collaborative-semantic space.
% 
Ablation studies confirm that each stage contributes: input anchoring and CAR jointly reshape the code space, CPA refines ranking precision, and semantic-aware reranking provides a complementary tie-breaking signal by re-evaluating candidates in the continuous collaborative-semantic space.
% 
Experiments on three Amazon datasets show that CAGRec outperforms all baselines on every metric, with NDCG@10 gains of $+2.8\%$ to $+3.7\%$ over the best multimodal generative baseline.
