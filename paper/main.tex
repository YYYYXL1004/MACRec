...
% 
Our codes are available at https://anonymous.4open.science/r/CAGRec.
\end{abstract}
...
Recommendation systems help users navigate information overload across e-commerce~\cite{amazonrec1,taobaorec2,din}, social media~\cite{davidson2010youtube,dnn}, and news services~\cite{news_rec,nrms}.
%
...
%
Later work improves tokenization via collaborative regularization~\cite{letter}, language model adaptation~\cite{LCRec}, or hierarchical decomposition~\cite{cofirec}.
%
...
%
As shown in Figure~\ref{fig:intro}, given a user who purchased an electric guitar, a tuner, and a guitar pick, content-only methods rank a guitar cable first (0.82) and a guitar case second (0.79)---both share surface-level textual or visual similarity with the guitar---while the ground truth amplifier ranks last (0.41), because its audio-electronics description and boxy form differ drastically from the guitar.
%
With collaborative anchoring, the amplifier rises to rank~\#1 (0.91) because co-purchase patterns anchor item representations, and the model also surfaces an effects pedal (0.80) that content-only methods entirely miss.
%
...
As illustrated in Figure~\ref{fig:model}, CAGRec injects collaborative signals at three stages.
%
...
& HR@10     & 0.0922 & 0.1250 & 0.1030 & \underline{0.1321} & 0.0994 & 0.1167 & 0.0959 & 0.1254 & 0.1245 & \textbf{0.1325} \\
...
% 
The statistics after preprocessing are summarized in Table~\ref{dataset}.
...
As shown in Table~\ref{tab:main}, we compare all methods across three datasets.
%
...
%
Beyond the NDCG@10 gains, CAGRec also achieves the highest HR@10 on all three datasets, with improvements of $+2.9\%$ on both Instruments and Games over MISSRec, confirming that collaborative anchoring helps the autoregressive model surface a broader set of relevant candidates.
...
\midrule
Concat (ours)     & \textbf{5.0 / 2.0} & \textbf{0.1161} & \textbf{0.1401} & \textbf{0.0995} & \textbf{0.1072} \\
\bottomrule
\end{tabular}
\end{table}

As shown in Table~\ref{tab:fusion}, we compare four fusion strategies on Instruments, all using the same SASRec collaborative embedding ($d_c\!=\!256$) and identical T5 configuration (including CPA).
%
...
As shown in Figure~\ref{fig:hyperparam}, we report HR@10 and NDCG@10 on Instruments as we vary four key hyperparameters one at a time.
