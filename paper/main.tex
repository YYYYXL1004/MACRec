...
\begin{figure}[t]
    ...
    \caption{Illustrative comparison on the Instruments dataset. (a)~User interaction history. (b)~Content-only methods fail to surface the ground truth (amplifier): text similarity favors a guitar cable and image similarity favors a guitar case. (c)~Collaborative anchoring correctly ranks the amplifier first and retrieves collaboratively related items that content-only methods miss.}
...
%
As shown in Figure~\ref{fig:intro}, given a user who purchased an electric guitar, a tuner, and a guitar pick, content-only methods rank a guitar cable first---its concise description centers on basic instrument-accessory properties, keeping the text embedding close to the guitar---and a guitar case second (high image similarity from its guitar-shaped silhouette), while the ground truth---a guitar amplifier---ranks last because its description is dominated by audio-electronics specifications (wattage, channels, EQ, reverb) that pull the text embedding away from the guitar's instrument-centric description, and its boxy form bears no visual resemblance.
%
...
We obtain collaborative embeddings ($d_c\!=\!256$) from a pretrained SASRec~\cite{sasrec} model, text features ($d_t\!=\!4096$) from LLaMA-7B~\cite{touvron2023llama} mean pooling, and image features ($d_v\!=\!768$) from ViT-L/14~\cite{clip}.
% 
